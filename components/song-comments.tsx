...
export function SongComments({
  ...
  const repliesByParent = useMemo(() => {
    const map = new Map<number, TrackComment[]>();
  ...
  async function remove(commentId: number) {
    ...
    try {
      const res = await fetch(
        `/api/tracks/${trackId}/comments?comment_id=${commentId}`,
        { method: "DELETE" },
      );
  ...
  async function toggleLike(commentId: number) {
    ...
    try {
      const res = await fetch(`/api/tracks/${trackId}/comments/like`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  function renderComment(c: TrackComment, isReply: boolean) {
    const canDelete = currentUserId === c.user_id || isTrackOwner;
    return (
      <div
        key={c.id}
  ...
  return (
    <section className="mt-8 border-t border-white/[0.08] pt-6">
      <h2 className="text-xs uppercase tracking-[0.16em] text-white/35">
        Comments
      ...
      {missingTable ? (
        <p className="mt-3 text-sm text-white/40">
          Run{" "}
          <code className="text-[#1DB954]">20260809_track_comments.sql</code> in
          Supabase to enable comments.
        </p>
      ) : null}

      {loadError || error ? (
        <p className="mt-3 text-sm text-[#1DB954]" role="alert">
          {error || loadError}
        </p>
      ) : null}
          ...
          <textarea
            ...
            placeholder={
              !signedIn
                ...
                : replyTo
                  ? "Write a reply…"
                  : "Say something about this track…"
            }
            disabled={pending}
      ...
      {!missingTable && comments.length === 0 && !loadError ? (
        <p className="mt-5 text-sm text-white/40">No comments yet — be first.</p>
      ) : null}

      {roots.length > 0 ? (
        <ul className="mt-5 space-y-3">
          {roots.map((c) => (
            <li key={c.id} className="space-y-2 list-none">
