...
import { NewSoundsClient } from "@/app/new-sounds/new-sounds-client";
...
export default async function NewSoundsPage() {
  ...
  const result = await loadNewSoundsTracks(supabase, 40);
  ...
  return (
    <main className="min-h-dvh bg-[#040d06] pb-28 text-[#f8f8f8]">
        ...
        <div className="mx-auto flex w-full max-w-5xl items-center justify-between px-5 py-4 sm:px-8">
          <div>
            ...
            <p className="mt-1 text-sm text-white/45">
              Fresh music launches on RECT — scheduled drops appear here when
              their launch date hits.
            </p>
          </div>
          <Link
            href="/new"
      ...
      <div className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8">
        <NewSoundsClient tracks={result.tracks} loadError={result.error} />
      </div>
    </main>
  );
}
