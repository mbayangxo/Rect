...
import type { PlaceHub } from "@/lib/dashboard/places";

const TONES = [
  "from-[#0F2B1A] to-[#060908]",
  "from-[#1A180F] to-[#080906]",
  "from-[#0F1A2B] to-[#06080A]",
  "from-[#2B1A0F] to-[#090806]",
  "from-[#14201a] to-[#060908]",
...
export function PlacesClient({ hubs, loadError, personalized }: Props) {
  return (
      ...
      <header className="border-b border-white/10">
        <div className="mx-auto flex w-full max-w-5xl items-center justify-between px-5 py-4 sm:px-8">
          ...
          <nav className="flex gap-4 text-sm text-white/55">
            ...
            <Link href="/places" className="text-[#1DB954]">
              Places
            </Link>
      ...
      <div className="mx-auto w-full max-w-5xl space-y-8 px-5 py-10 sm:px-8">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm text-white/45">
            {personalized
              ? "Your places rise first — artists and published tracks from each scene."
              : "Scenes from artists’ places, filled with published tracks."}
          </p>
        </div>
        ...
        {hubs.length === 0 && !loadError ? (
          <div className="rounded-2xl border border-dashed border-white/15 px-6 py-14 text-center">
            ...
            <p className="mt-2 text-sm text-white/40">
              Artists with places in their profile will appear here.
            </p>
          </div>
        ) : (
            ...
            {hubs.map((h, i) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-white/45">
                  {h.artist_count}{" "}
                  {h.artist_count === 1 ? "artist" : "artists"}
                  {" · "}
                  {h.track_count}{" "}
