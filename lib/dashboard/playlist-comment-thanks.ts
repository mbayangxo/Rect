...
export const PLAYLIST_COMMENT_THANKS_MAX = 280;

export async function sendPlaylistCommentThanks(
  ...
  if (!trimmed || trimmed.length > PLAYLIST_COMMENT_THANKS_MAX) {
    return {
      ok: false,
      error: `Thanks must be 1–${PLAYLIST_COMMENT_THANKS_MAX} characters`,
  ...
  const { data, error } = await supabase.rpc("send_playlist_comment_thanks", {
  ...
  if (error) {
    if (isMissingRelation(error.message)) {
      return {
        ok: false,
        error: "Run playlist comment thanks SQL in Supabase first",
        code: "missing_table",
      };
    }
    ...
    const code = /not_authenticated/i.test(msg)
      ...
      : /comment_not_found/i.test(msg)
        ...
        : /cannot_thank_self/i.test(msg)
          ? "cannot_thank_self"
          : /not_playlist_owner|not_allowed/i.test(msg)
            ? "not_allowed"
            : /blocked/i.test(msg)
              ? "blocked"
...
/** Map playlist comment_id → thanks message for the current thanker. */
export async function loadMyPlaylistCommentThanksMap(
  supabase: SupabaseClient,
  thankerId: string,
  commentIds: number[],
